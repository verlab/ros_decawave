...
#
# (c) 2016-2017, LEAPS - All rights reserved.
#
...
recover_s132: $(RECOVERY_DIR)s132_nrf52_3.0.0_softdevice.hex
	$(OPENOCD) -d0 -s $(OPENOCD_DIR) -f $(OPENOCD_CFG) -c "$(CMSIS_DAP_SERIAL)init;reset halt;sleep 250;flash erase_address 0 0x1f000;reset halt;wdt_feed;flash write_image $< 0;reset run;shutdown"

recover_bldr: $(RECOVERY_DIR)bootloader_s132.bin
	$(OPENOCD) -d0 -s $(OPENOCD_DIR) -f $(OPENOCD_CFG) -c "$(CMSIS_DAP_SERIAL)init;reset halt;sleep 250;flash erase_address 0x1f000 0x1000;reset halt;wdt_feed;flash write_image $< 0x1f000;reset run;shutdown"

recover_fw1: $(RECOVERY_DIR)dwm-core_fw1.bin
	$(OPENOCD) -d0 -s $(OPENOCD_DIR) -f $(OPENOCD_CFG) -c "$(CMSIS_DAP_SERIAL)init;reset halt;sleep 250;flash erase_address 0x22000 0x22000;reset halt;wdt_feed;flash write_image $< 0x22000;reset run;shutdown"

recover_fw2: $(RECOVERY_DIR)dwm-core_fw2.bin
	$(OPENOCD) -d0 -s $(OPENOCD_DIR) -f $(OPENOCD_CFG) -c "$(CMSIS_DAP_SERIAL)init;reset halt;sleep 250;flash erase_address 0x44000 0x3c000;reset halt;wdt_feed;flash write_image $< 0x44000;reset run;shutdown"
